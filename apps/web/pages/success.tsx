...
import { SUCCESS_IMG } from "@calcom/lib/constants";
...
export default function Success(props: SuccessProps) {
  ...
  const location: ReturnType<typeof getEventLocationValue> = Array.isArray(
    _location
  )
    ? _location[0] || ""
    : _location || "";
  ...
  const [date, setDate] = useState(
    dayjs.utc(asStringOrThrow(router.query.date))
  );
  const { eventType, bookingInfo }: any = props;
  ...
  const needsConfirmation =
    eventType.requiresConfirmation && reschedule != "true";
  ...
  const userIsOwner = !!(
    session?.user?.id &&
    eventType.users.find((user) => (user.id = session.user.id))
  );
  ...
  return (
    <div className={isEmbed ? "" : "h-screen"} data-testid="success-page">
      ...
      <CustomBranding
        lightVal={props.profile.brandColor}
        darkVal={props.profile.darkBrandColor}
      />
      <main
        className={classNames(
          shouldAlignCentrally ? "mx-auto" : "",
          isEmbed ? "max-w-3xl min-h-screen flex items-center" : "max-w-3xl"
        )}
      >
        <div
          className={classNames(
        ...
        >
          {isSuccessRedirectAvailable(eventType) &&
          ...
          >
            ...
            >
              <div
                className={classNames(
                  "inline-block transform overflow-hidden rounded-md border sm:my-8 sm:max-w-lg bg-black p-4",
                  isBackgroundTransparent
                    ? ""
                    : "dark:bg-darkgray-100 bg-black bg-opacity-25 dark:border-neutral-700",
              ...
              >
                  ...
                  {!isCancelled ? (
                    ...
                    >
                      ...
                      {needsConfirmation && !isCancelled && (
                        <Icon.FiClock className="h-8 w-8 text-green-600" />
                      )}
                      {isCancelled && (
                        <Icon.FiX className="h-8 w-8 text-red-600" />
                      )}
                    </div>
                  )}
                  <div className="mt-3 text-center sm:mt-5">
                    <h3
                      className="text-2xl font-semibold leading-6 text-neutral-500 text-white"
                      id="modal-headline"
                    >
                      {
                        needsConfirmation && !isCancelled
                          ...
                          ? t("meeting_is_scheduled_second")
                          : t("meeting_is_scheduled")
                        //(bookingInfo?.paid ? '': 'Your performance is not Booked')
                      }
                    </h3>
                    <div className="mt-3">
                    ...
                    <div className="border-bookinglightest dark:border-darkgray-200 mt-4 grid md:grid-cols-3 border-t border-b py-4 text-left dark:text-gray-300 text-white">
                      ...
                      <div className="col-span-2 mb-6">
                        <RecurringBookings
                          ...
                          listingStatus={
                            (listingStatus as string) || "recurring"
                          }
                          date={date}
                          is24h={is24h}
                        />
                      ...
                      {bookingInfo.coupon && (
                        <>
                          <div className="font-medium">{t("coupon")}</div>
                          <div className="col-span-2 mb-6">
                            {bookingInfo.coupon}
                          </div>
                        </>
                      ...
                      {bookingInfo?.description && (
                        <>
                          ...
                          <div className="col-span-2 mb-2 md:mt-9">
                ...
                {!needsConfirmation &&
                  ...
                  (!isCancellationMode ? (
                    <div className="border-bookinglightest text-bookingdark dark:border-darkgray-200 mt-2 grid-cols-3 border-b py-4 text-left sm:grid">
                      <span className="font-medium text-gray-700 ltr:mr-2 rtl:ml-2 dark:text-gray-900">
                        {t("need_to_make_a_change")}
                      </span>
                      <div
                        className={classNames(
                          "col-span-2 items-center dark:text-gray-700 text-white",
                        ...
                        {!props.recurringBookings && (
                          <>
                            <div className="underline">
                              <Link href={"/reschedule/" + bookingInfo?.uid}>
                                {t("reschedule")}
                              </Link>
                            </div>
                          </>
                    ...
                    <CancelBooking
                      ...
                      theme={
                        isSuccessBookingPage ? props.profile.theme : "light"
                      }
                      session={session}
                    />
                  ))}
                {userIsOwner &&
                  ...
                  !isCancellationMode &&
                  !isCancelled && (
                    <div className="border-bookinglightest text-bookingdark dark:border-darkgray-200 mt-2 grid-cols-3 border-b py-4 text-left hidden">
                      <span className="flex self-center font-medium text-gray-700 ltr:mr-2 rtl:ml-2 dark:text-gray-900">
                        {t("add_to_calendar")}
                      </span>
                      <div className="justify-left mt-1 flex flex-grow text-left sm:mt-0">
                        ...
                        >
                          <a className="mr-2 h-10 w-10 rounded-sm border border-neutral-200 px-3 py-2 dark:border-neutral-700 text-white">
                            <svg
                        ...
                        >
                          <a
                            className="mx-2 h-10 w-10 rounded-sm border border-neutral-200 px-3 py-2 dark:border-neutral-700 text-white"
                            target="_blank"
                          >
                            <svg
                ...
                <div className="border-bookinglightest text-booking-lighter dark:border-darkgray-200 pt-4 text-center text-xs text-white">
                  *Booking is refundable up to 4 hours before the performance.
                </div>
              </div>
            </div>
          </div>
      ...
      {eventType.recurringEvent?.count &&
        recurringBookingsSorted.slice(0, 4).map((dateStr, idx) => (
          <div key={idx} className="mb-2">
            ...
            <br />
            {dayjs(dateStr).format("LT")} -{" "}
            {dayjs(dateStr).add(eventType.length, "m").format("LT")}{" "}
...
export async function getServerSideProps(context: GetServerSidePropsContext) {
  ...
  const eventTypeRaw = !eventTypeId
  ...
  const profile = {
    ...
    darkBrandColor: eventType.team
      ? null
      : eventType.users[0].darkBrandColor || null,
  ...
  return {
    props: {
      hideBranding: eventType.team
        ? eventType.team.hideBranding
        : isBrandingHidden(eventType.users[0]),
      profile,
      eventType,
      recurringBookings: recurringBookings
        ? recurringBookings.map((obj) => obj.startTime.toString())
        : null,
