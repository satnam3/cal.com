...
export default function Type(props: inferSSRProps<typeof getServerSideProps>) {
  ...
  const [error, setError] = useState<string | null>(
    props.booking ? null : t("booking_already_cancelled")
  );
  ...
  return (
    <div className="h-screen bg-neutral-100 dark:bg-neutral-900">
      <HeadSeo
        title={`${t("cancel")} ${props.booking && props.booking.title} | ${
          props.profile?.name
        }`}
        description={`${t("cancel")} ${
          props.booking && props.booking.title
        } | ${props.profile?.name}`}
      />
      <CustomBranding
      ...
      <main className="h-full sm:flex sm:items-center">
        <div className="mx-auto flex justify-center px-4 pt-4 pb-20 sm:block sm:p-0">
          <div className="inline-block transform overflow-hidden rounded-md border bg-black bg-opacity-25 px-8 pt-5  pb-4 text-left align-bottom transition-all dark:border-neutral-700 dark:bg-gray-800 sm:my-8 sm:w-full sm:max-w-lg sm:py-6 sm:align-middle">
            <div>
              <div>
                {error && (
                  <div>
                ...
                {!error && (
                  <>
                    <div>
                      ...
                      <div className="mt-3 text-center sm:mt-5">
                        {/* <h3 className="text-2xl font-semibold leading-6 text-neutral-700 text-white">
                        ...
                        <div className="border-bookinglightest mt-4 grid grid-cols-3 border-t py-4 text-left dark:border-gray-900 text-white">
                            ...
                            props.recurringInstances ? (
                              <>
                                <div className="mb-1 inline py-1 text-left">
                                  <div>
                                    {dayjs(
                                      props.recurringInstances[0].startTime
                                    ).format(
                                    ...
                                    >
                                      ...
                                      >
                                        {t("plus_more", {
                                          count:
                                            props.recurringInstances.length - 1,
                                        })}
                                      </CollapsibleTrigger>
                                      <CollapsibleContent>
                                        {props.booking?.eventType.recurringEvent
                    ...
                    {props.cancellationAllowed && (
                      <div>
                        <textarea
                          ...
                          placeholder={
                            props.booking.status === "ACCEPTED"
                              ? t("cancellation_reason_placeholder_artist")
                              : t("cancellation_reason_placeholder")
                          }
                          value={cancellationReason}
                          onChange={(e) =>
                            setCancellationReason(e.target.value)
                          }
                          className="mt-2 mb-3 w-full dark:border-gray-900 dark:bg-gray-700 sm:mb-3 "
                          rows={3}
                        />
                        <div
                          className={`flex ${
                            data?.user?.id !== props.booking.user.id
                              ? "justify-between"
                              : "justify-end"
                          } space-x-2 text-center rtl:space-x-reverse`}
                        >
                          {data?.user?.id !== props.booking.user.id &&
                            !props.booking.eventType?.recurringEvent && (
                              <Button
                          ...
                          <Button
                            ...
                            onClick={async () => {
                              ...
                              telemetry.event(
                                telemetryEventTypes.bookingCancelled,
                                collectPageParameters()
                              );
                              ...
                              if (res.status >= 200 && res.status < 300) {
                                await router.push(
                                  `/cancel/success?name=${
                                    props.profile.name
                                  }&title=${props.booking.title}&eventPage=${
                                    props.profile.slug
                                  }&team=${
                                ...
                                setLoading(false);
                                setError(
                                  `${t("error_with_status_code_occured", {
                                    status: res.status,
                                  })} ${t("please_try_again")}`
                                );
                              }
...
export const getServerSideProps = async (
  ...
  const addHours = (numOfHours: number) => {
    let date = new Date();
  ...
  return {
    props: {
      ...
      recurringInstances,
      cancellationAllowed:
        (!!session?.user && session.user?.id === booking.user?.id) ||
        booking.startTime >= addHours(4),
