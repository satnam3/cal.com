  ...
  return (
    <Link
     href={`/event-types/${type.id}?tabName=setup`}
        ...
        <div>
            <span
              className="truncate font-medium text-neutral-700 ltr:mr-1 rtl:ml-1"
  ...
  */
  ...
  return (
    <div className="-mx-4 mb-16 overflow-hidden rounded-sm border border-gray-200 sm:mx-0">
      <ul ref={parent} className="divide-y divide-neutral-200" data-testid="event-types">
                ...
                <MemoizedItem type={type} group={group} readOnly={readOnly} />
                  <div className="mt-4 hidden flex-shrink-0 sm:mt-0 sm:ml-5 sm:flex">
                    <div className="flex justify-between space-x-2 rtl:space-x-reverse">
                      ...
                      <div className={classNames("flex justify-between space-x-2 rtl:space-x-reverse ", type.$disabled && "pointer-events-none cursor-not-allowed")}>
                    ...
                    <ButtonGroup combined>
                      <Tooltip content={t("preview")}>
                          <Button
                            ...
                            variant="icon"
                            className={classNames(
                              !type.$disabled && "group-hover:text-white"
                            )}
                            href={calLink}
    ...
    <div className="mb-4 flex items-center space-x-2">
      ...
      <div>
        ...
        {membershipCount && teamId && (
          <span className="relative -top-px text-xs text-neutral-500 ltr:ml-2 rtl:ml-2">
...
const EventTypesPage = () => {
  ...
  useEffect(() => {
    if (data?.user) {
      const getUsetiful = async () => {
        const value = await fetch("/api/usetiful", {
          method: "POST",
          headers: {
    ...
    if (router?.query?.email && data) {
      const checkEmail = async () => {
        await fetch("/api/check-email", {
          ...
          body: JSON.stringify({
            urlEmail: router.query.email,
            existsEmail: data.user.email,
          }),
        }).then(async (res) => {
          const response = await res.json();
          if (!response.message) {
            signOut();
            if (router?.query?.stripe?.includes("true")) {
              router.push(
                `/auth/login?email=${router?.query?.email}&stripe=true`
              );
            } else {
              router.push(`/auth/login?email=${router?.query?.email}`);
            }
          } else {
            if (router?.query?.tours) {
              const tou = JSON.parse(decodeURI(router.query.tours));

              window.usetifulTags = {
                userId: router.query.email,
                welcome: tou.tour_welcome,
                profile: tou.tour_profile,
                event: tou.tour_event,
                stripe: tou.tour_stripe,
                availability: tou.tour_availability,
              };
            } else {
              window.usetifulTags = {
                userId: router.query.email,
            ...
            if (router?.query?.stripe?.includes("true")) {
              router.replace("/apps/stripe");
            } else {
              router.replace(response.link);
            }
          }
        });
      };
  ...
  }, [router.query.email, data, router]);
*/
  const isMobile = useMediaQuery("(max-width: 768px)");

  return (
    <div>
      <Shell heading={t("event_types_page_title")} subtitle={t("event_types_page_subtitle")} custom={true} CTA={<CTA />}>
        <WithQuery
